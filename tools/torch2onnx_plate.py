...
def model_converter():
    model = Model()
    model.load_state_dict(torch.load("../work_dirs_plate/model_epoch_2.pth", map_location=torch.device('cpu'))['model_state_dict'])
    ...
    input_names = ['input']
    output_names = ['output']
    torch.onnx.export(model, dummy_input, 'model_plate.onnx',
                      export_params=True,
                      verbose=True,
